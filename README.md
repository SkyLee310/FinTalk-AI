...
- **The whiteboard diagram is not drawn.** A photographed board is uploaded alongside the recording, extracted, redacted, stored and shown on the meeting — but as Mermaid source rather than a rendered picture. Rendering needs a client-side diagram library; the source is what was actually stored and what an auditor reads.
